  ...
  return (
    <div className="space-y-10 max-w-7xl mx-auto pb-20">
      ...
      <PageIntro
        ...
        title="Release Shield"
        description="Protect verified savings in your release pipeline. Enforce total transfer bytes, image category ceilings, and essential task assertions in local CLI and GitHub Actions."
      />
      ...
      {notification && (
        <div role="status" aria-live="polite" className="p-3 rounded-xl bg-surface border border-lime/30 text-lime text-xs font-mono">
          {notification}
        </div>
      )}
      ...
      <Card className="p-5 sm:p-8 glass-panel-elevated border border-surface-border space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-surface-border/60 pb-4">
          <SectionHeading
            ...
            title="Test your release budget."
            description={<>Target: Campus event registration journey · Active ceiling: {byteCeilingKb} KB transfer · Mode: {strictMode ? "Strict (exit nonzero)" : "Warn"}</>}
          />
        ...
        {isEvaluating && <p role="status" aria-live="polite" className="text-sm text-sage">Evaluating the {evalVariant} journey against your current budget…</p>}
        {evalError && <div role="alert" className="rounded-2xl border border-red-500/40 bg-red-950/40 p-4 text-sm text-red-200 flex items-start gap-2"><AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />{evalError}</div>}
        <AnimatePresence>
          {evalResult && (
            <motion.div
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-surface-border/60 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-cream font-bold">CLI budget gate output</span>
                </div>
                <Badge
      ...
      <Reveal className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 min-w-0 space-y-6">
          <Card className="p-6 glass-panel-elevated border border-surface-border space-y-6 font-mono text-xs">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap justify-between gap-2">
                <label htmlFor="transfer-budget" className="text-cream font-medium">Total transfer budget ceiling</label>
              ...
              <input
                ...
                step="50"
                value={byteCeilingKb}
                aria-valuetext={`${byteCeilingKb} kilobytes`}
              ...
              />
              <div className="flex flex-wrap justify-between gap-2 text-[10px] text-sage/60">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap justify-between gap-2">
                <label htmlFor="image-budget" className="text-cream font-medium">Image category transfer limit</label>
              ...
              <input
                ...
                step="50"
                value={imageLimitKb}
                aria-valuetext={`${imageLimitKb} kilobytes`}
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap justify-between gap-2">
                <label htmlFor="request-budget" className="text-cream font-medium">Maximum HTTP requests</label>
              ...
              <input
                ...
                step="1"
                value={requestLimit}
                aria-valuetext={`${requestLimit} requests`}
            ...
            <div className="pt-4 border-t border-surface-border flex flex-wrap items-center justify-between gap-3">
              <div>
                <label htmlFor="strict-gating" className="text-cream font-medium">Strict CI gating</label>
                <div id="strict-gating-help" className="text-[11px] text-sage/60">Exit nonzero on budget breach to block PR merge</div>
              </div>
              <input
                id="strict-gating"
                aria-describedby="strict-gating-help"
                type="checkbox"
                checked={strictMode}
          ...
          <Card className="p-6 glass-panel border border-surface-border space-y-4 font-mono text-xs">
            <h3 className="font-display text-xl text-cream flex items-center gap-2">
              <GitBranch className="w-4 h-4 text-lime" /> Install in three steps
            </h3>
            <ol className="space-y-3 list-decimal list-inside text-sage/80 leading-relaxed text-[11px]">
              <li>
                ...
                <pre className="overflow-x-auto mt-1 p-2 rounded bg-black/60 text-lime font-mono text-[10px]">
        ...
        <div className="lg:col-span-7 min-w-0 space-y-4">
          <Card className="p-6 glass-panel-elevated border border-lime/30 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 font-mono text-xs border-b border-surface-border pb-3">
              <span className="text-cream font-bold flex items-center gap-2 min-w-0 [overflow-wrap:anywhere]">
            ...
            <pre tabIndex={0} aria-label="Generated GitHub Actions workflow" className="p-4 rounded-xl bg-black/80 border border-surface-border overflow-x-auto text-[11px] font-mono text-lime/90 leading-relaxed max-h-[500px]">
              {workflowYaml}
            </pre>
          </Card>
        </div>
